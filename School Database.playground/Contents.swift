...
struct Student{
    let name: String
    let favoriteTeacherIDproperty: Int
}
...
struct School{
    ...
    func printFavoriteTeacherInfo(for: String){
        if let getTeacher(withID:Int) == Teacher? {
            print("\(teachers.name) , \(teachers.hairColor ?? ), \(teachers.email)")
        }
        print("Sorry, this teacher cannot be found")
    }
